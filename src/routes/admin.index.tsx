  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
        ...
        <Stat icon={Film} label="Movies" value={data.counts.movies} />
        <Stat icon={Code2} label="Codes" value={data.counts.codes} />
        <Stat icon={Eye} label="Views" value={data.counts.views} />
        <Stat icon={Inbox} label="Pending" value={data.counts.pendingRequests} />
        <Stat icon={Send} label="Channel posts" value={data.counts.channelPosts} />
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="glass-card rounded-xl p-6">
          ...
          <ul className="space-y-2">
            ...
            {!data.topMovies.length && <p className="text-sm text-muted-foreground">No data yet.</p>}
          </ul>
        </div>
        <div className="glass-card rounded-xl p-6">
          ...
          <ul className="space-y-2">
            ...
            {!data.latestUsers.length && <p className="text-sm text-muted-foreground">No users yet.</p>}
          </ul>
        </div>
      </div>
    </div>
